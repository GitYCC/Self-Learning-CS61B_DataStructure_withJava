...
public class Set {
	...
	public void insert(Comparable c) {
        try{
            if (myList.length()==0) {
                ...
                for(int i=0;i<myList.length();i++) {
                    ...
                    if (i == 0) {
          			...
          			if(item.compareTo(c)==0) {
            			break;
          			} else if (item.compareTo(c)>0) { // cross equal line
          		  		node.insertBefore(c);
          		  		break;
          			} else if (i == myList.length()-1) { // already scan all myList
            			node.insertAfter(c);
            			break;
          			}
        		}
      		...
      		System.err.println("Set: insert() failed.");
      		lbe.printStackTrace(System.err);
    	}
  	}
	...
	public void union(Set s) {    
    	try{
      		DListNode node = null;
      		DListNode node_s = null;
      		int index_myList = 0;
      		int size_myList = this.cardinality();
      		Comparable item;
      		Comparable item_s;
	      	...
	      	for (int i=0;i<s.cardinality();i++) {
	        	if (i==0) {
	          		node_s = (DListNode)(s.myList.front());
	        	} else {
	          		node_s = (DListNode)(node_s.next());
	        	}
				
				if (index_myList==size_myList){
	          		myList.insertBack(node_s.item());
	        	}
	        	
	        	while (index_myList<size_myList) {
	          		if (index_myList==0) {
	            		node = (DListNode)(myList.front());
	          		} else if (skipForward){
	          			skipForward = false;
	          		...
	          		item = (Comparable)(node.item());
	          		item_s = (Comparable)(node_s.item());
	          		
	          		if (item.compareTo(item_s)==0) {
	          			index_myList++;
	          			break;
	          		} else if(item.compareTo(item_s)>0){ // cross equal line
	            		node.insertBefore(node_s.item());
	            		skipForward = true;
	            		break;
	          		} else {
	          			index_myList++;
      		...
      		System.err.println("Set: union failed.");
      		lbe.printStackTrace(System.err);
	    }
  	}
	...
	public void intersect(Set s) {
		...
		try{
	      	...
	      	for (int i=0;i<s.cardinality();i++) {
	        	...
	        	while (index_myList<size_myList) {
	          		...
	          		if (item.compareTo(item_s)==0) {	
	          			index_myList++;
	          			break;
	          		} else if (item.compareTo(item_s)>0) {
	          			skipForward = true;
	            		break;
	          		} else {
	          			index_myList++;
      		...
      		System.err.println("Set: intersect failed.");
      		lbe.printStackTrace(System.err);
	    }
	...
	public String toString() {
		...
		try {
			if (myList.length() > 0) {
				DListNode node = (DListNode) (myList.front());
				for (int i = 0; i < myList.length(); i++) {
					msg += node.item().toString() + "  ";
					node = (DListNode) (node.next());
				}
			}
		} catch (InvalidNodeException lbe) {
