...
public class RunIterator implements Iterator {
	 ...
	 */
	private DoublyLinkedNode<RunLengthEncoding.Elem> now_pointer;
	...
	// of the encoding.
	RunIterator(DoublyLinkedNode<RunLengthEncoding.Elem> head) {
		this.now_pointer = head;
	}
	...
	public boolean hasNext() {
		if (this.now_pointer.getNext().getElement()==null) {
			return false;
		} else {
			return true;
		}
	...
	public int[] next() {
		...
		if (!this.hasNext()) {
			...
			output[0] = this.now_pointer.getNext().getElement().getRunLengths();
			output[1] = (int) this.now_pointer.getNext().getElement().getRed();
			output[2] = (int) this.now_pointer.getNext().getElement().getGreen();
			output[3] = (int) this.now_pointer.getNext().getElement().getBlue();

			this.now_pointer = this.now_pointer.getNext();
			return output;
		}
	}
