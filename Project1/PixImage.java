...
public class PixImage {
	...
	public PixImage(int width, int height) {
		if (width < 1 && height < 1) {
			System.out.println("Error: width and height should be positive");
			System.exit(1);
		}
	...
	public String toString() {
		...
		for (int y = 0; y < this.height; y++) {
			for (int x = 0; x < this.width; x++) {
				red = String.format("%02x", this.image[x][y][0] & 0xFF);
				green = String.format("%02x", this.image[x][y][1] & 0xFF);
				blue = String.format("%02x", this.image[x][y][2] & 0xFF);
	...
	public PixImage boxBlur(int numIterations) {
		...
		if (numIterations > 0) {
			short temp_image[][][] = this.image;
			short new_image[][][] = null;
			int rr = 0, gg = 0, bb = 0, num = 0;

			for (int i = 0; i < numIterations; i++) {
				new_image = new short[this.width][this.height][3];
				for (int y = 0; y < this.height; y++) {
					for (int x = 0; x < this.width; x++) {
						rr = 0;
						gg = 0;
						...
						for (int dx = -1; dx <= 1; dx++) {
							for (int dy = -1; dy <= 1; dy++) {
								if (x + dx >= 0 && x + dx < this.width && y + dy >= 0 && y + dy < this.height) {
									rr += temp_image[x + dx][y + dy][0];
									gg += temp_image[x + dx][y + dy][1];
									bb += temp_image[x + dx][y + dy][2];
									num++;
								}
							}
						}
						new_image[x][y][0] = (short) (rr / num);
						new_image[x][y][1] = (short) (gg / num);
						new_image[x][y][2] = (short) (bb / num);
					}
				}
	...
	public PixImage sobelEdges() {
		...
		int[][] XSobelOperator = { { 1, 0, -1 }, { 2, 0, -2 }, { 1, 0, -1 } };
		int[][] YSobelOperator = { { 1, 2, 1 }, { 0, 0, 0 }, { -1, -2, -1 } };
		int gx_r = 0, gy_r = 0, gx_g = 0, gy_g = 0, gx_b = 0, gy_b = 0;
		short gray = 0;
		
		for (int y = 0; y < this.height; y++) {
			for (int x = 0; x < this.width; x++) {
				gx_r = 0; gy_r = 0; gx_g = 0; gy_g = 0; gx_b = 0; gy_b = 0;
				for (int dx = -1; dx <= 1; dx++) {
					for (int dy = -1; dy <= 1; dy++) {
						if (x + dx >= 0 && x + dx < this.width && y + dy >= 0 && y + dy < this.height) {
							gx_r += this.image[x+dx][y+dy][0]*XSobelOperator[dx+1][dy+1];
							gy_r += this.image[x+dx][y+dy][0]*YSobelOperator[dx+1][dy+1];
							gx_g += this.image[x+dx][y+dy][1]*XSobelOperator[dx+1][dy+1];
							gy_g += this.image[x+dx][y+dy][1]*YSobelOperator[dx+1][dy+1];
							gx_b += this.image[x+dx][y+dy][2]*XSobelOperator[dx+1][dy+1];
							gy_b += this.image[x+dx][y+dy][2]*YSobelOperator[dx+1][dy+1];
						} else if ( dx==0 || dy==0 || ( (x+dx<0 || x+dx>=this.width) && (y+dy<0 || y+dy>=this.height) ) ) {
							gx_r += this.image[x][y][0]*XSobelOperator[dx+1][dy+1];
							gy_r += this.image[x][y][0]*YSobelOperator[dx+1][dy+1];
							gx_g += this.image[x][y][1]*XSobelOperator[dx+1][dy+1];
							gy_g += this.image[x][y][1]*YSobelOperator[dx+1][dy+1];
							gx_b += this.image[x][y][2]*XSobelOperator[dx+1][dy+1];
							gy_b += this.image[x][y][2]*YSobelOperator[dx+1][dy+1];							
						} else if ( (x+dx<0 || x+dx>=this.width) && !(y+dy<0 || y+dy>=this.height) ) {
							gx_r += this.image[x][y+dy][0]*XSobelOperator[dx+1][dy+1];
							gy_r += this.image[x][y+dy][0]*YSobelOperator[dx+1][dy+1];
							gx_g += this.image[x][y+dy][1]*XSobelOperator[dx+1][dy+1];
							gy_g += this.image[x][y+dy][1]*YSobelOperator[dx+1][dy+1];
							gx_b += this.image[x][y+dy][2]*XSobelOperator[dx+1][dy+1];
							gy_b += this.image[x][y+dy][2]*YSobelOperator[dx+1][dy+1];	
						} else if ( !(x+dx<0 || x+dx>=this.width) && (y+dy<0 || y+dy>=this.height) ) {
							gx_r += this.image[x+dx][y][0]*XSobelOperator[dx+1][dy+1];
							gy_r += this.image[x+dx][y][0]*YSobelOperator[dx+1][dy+1];
							gx_g += this.image[x+dx][y][1]*XSobelOperator[dx+1][dy+1];
							gy_g += this.image[x+dx][y][1]*YSobelOperator[dx+1][dy+1];
							gx_b += this.image[x+dx][y][2]*XSobelOperator[dx+1][dy+1];
							gy_b += this.image[x+dx][y][2]*YSobelOperator[dx+1][dy+1];
						}
					}
				}
				gray = this.mag2gray((long) (gx_r*gx_r+gy_r*gy_r+gx_g*gx_g+gy_g*gy_g+gx_b*gx_b+gy_b*gy_b));
				newPixImage.setPixel(x, y, gray, gray, gray);
			}
		}	
		return newPixImage;
