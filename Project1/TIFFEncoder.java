...
public class TIFFEncoder {
	...
	/**
	 * The TiffType enum represents the type of a value of a TIFF field, which
	 * can have a variety of sizes.
	 */
	 ...
	 *
	 * @param type
	 *            the type whose integer flag we want.
	...
	/**
	 * writeLeftAlignedValue() writes a given value of the given type into a
	 * given stream as a left-justified four-byte record. See Section 2 (page
	 * 15) of the TIFF spec for details.
	 *
	 * @param stream
	 *            the stream representing the file being written.
	 * @param type
	 ...
	 * @throws IOException
	 */
	private static void writeLeftAlignedValue(FileImageOutputStream stream, TiffType type, int val) throws IOException {
		switch (type) {
		case SHORT:
	 ...
	 * fits into the Value Offset. See Section 2 (page 15) of the TIFF spec for
	 * more details.
	 *
	 * @param stream
	 *            the stream representing the file being written.
	 ...
	 * @throws IOException
	 */
	private static void writeValueTag(FileImageOutputStream stream, int tag, TiffType type, int value)
			throws IOException {
	 ...
	 * does not fit into the Value Offset, so we store it at another offset. See
	 * Section 2 (page 15) of the TIFF spec for more details.
	 *
	 * @param stream
	 *            the stream representing the file being written.
	 * @param tag
	 ...
	 * @param offset
	 *            the offset in the file where the actual value is stored.
	 * @throws IOException
	 */
	private static void writeOffsetTag(FileImageOutputStream stream, int tag, TiffType type, int count, int offset)
			throws IOException {
		stream.writeShort(tag);
		stream.writeShort(getTypeInt(type));
	...
	/**
	 * writeTIFF() writes the specified data into a TIFF file. For more details,
	 * see the TIFF spec at
	 * http://partners.adobe.com/public/developer/en/tiff/TIFF6.pdf. This code
	 * adapted from http://paulbourke.net/dataformats/tiff/.
	 *
	 * @param data
	 *            the bytes of the image data.
	 * @param width
	 ...
	 * @param isCompressed
	 *            true if the data is compressed in PackBits format; false if it
	 *            is stored uncompressed.
	 */
	private static void writeTIFF(ArrayList<Short> data, int width, int height, String filename, boolean isCompressed) {
		...
		try {
			...
			stream.writeInt(data.size() + HEADER_SIZE); // Offset of image file
														// dir.
			...
			stream.writeShort(NUM_DIRS); // Number of image file directory
											// entries.
			...
			writeOffsetTag(stream, 258, TiffType.SHORT, 3, data.size() + HEADER_SIZE + DIR_SIZE * NUM_DIRS + 6);
			...
			// IFD entry 4: Photometric tag. 2 means it's a full-color RGB
			// image.
			...
			// IFD entry 7: Rows per strip. Our image is encoded as just one
			// strip.
			...
			// IFD entry 9: Planar configuration. 1 means each pixel is
			// continuous
	 ...
	 *
	 * @param rle
	 *            a run-length encoding of the image data.
	 * @param filename
	 *            the name of the file to write.
	 */
	public static void writeTIFF(RunLengthEncoding rle, String filename) {
		...
		for (RunIterator it = rle.iterator(); it.hasNext();) {
			...
			// The TIFF format can compress repeated bytes, so it can express a
			// run
			// of grayscale values in compressed form; but it cannot compress
			// repeated red-green-blue triples if the red, green, and blue
			// values are
			...
			if (run[1] == run[2] && run[1] == run[3]) {
				// It's a grayscale run. We can write the run in a compressed
				// format.
				int i = 0;
				while (i < run[0] * 3) { // run[0] is the number of pixels in
											// the run.
					// Figure the number of bytes to write in one run. Note that
					// it is
					// always a factor of 3.
					int curCount = Math.min(Math.min(run[0] * 3 - i, 126), (rle.getWidth() - currentX) * 3);
					pixels.add((short) (1 - curCount)); // # of times value is
														// repeated.
					pixels.add((short) run[1]); // The value that is repeated.

					// The TIFF format does not allow you to compress across row
					// boundaries, so we must keep track of the current column
					// so we can
				...
				while (i < run[0] * 3) { // run[0] is the number of pixels in
											// the run.
					// Figure the number of bytes to write in one literal. Note
					// that it
					// is always a factor of 3.
					int curCount = Math.min(Math.min(run[0] * 3 - i, 126), (rle.getWidth() - currentX) * 3);
					pixels.add((short) (curCount - 1)); // Number of literal
														// values.
					for (int j = 0; j < curCount / 3; j++) { // The literal
																// values.
